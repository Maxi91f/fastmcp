...
- **`on_disconnect`** is called when the session context manager exits (client disconnects). It also **returns `None`** and cannot modify the disconnection process. This hook is useful for cleanup operations, closing resources, logging disconnections, or persisting session data.
</Note>
...
class SessionLifecycleMiddleware(Middleware):
    ...
    async def on_disconnect(self, context: MiddlewareContext[None], call_next):
