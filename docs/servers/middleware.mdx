...
- `on_disconnect`: Called when a client disconnects and the session is closing (returns `None`)
<Note>
The `on_initialize` and `on_disconnect` hooks provide lifecycle management for client sessions:
- **`on_initialize`** receives the client's initialization request but **returns `None`** rather than a result. The initialization response is handled internally by the MCP protocol and cannot be modified by middleware. This hook is useful for client detection, logging connections, or initializing session state.
- **`on_disconnect`** is called when the session context manager exits (client disconnects). It also **returns `None`** and cannot modify the disconnection process. This hook is useful for cleanup operations, closing resources, logging disconnections, or persisting session data. Receives a pseudo-request with `params` containing the client's initialization parameters (capabilities, clientInfo), but has limited context (no request context).
</Note>
...
Track client connections and perform cleanup operations using the `on_initialize` and `on_disconnect` hooks:
...
class SessionLifecycleMiddleware(Middleware):
    ...
    async def on_initialize(
        ...
        if context.fastmcp_context:
            ...
            print(f"Client connected: {client_name} (session: {session_id})")
        
        return await call_next(context)
    
    async def on_disconnect(self, context: MiddlewareContext, call_next):
...
mcp.add_middleware(SessionLifecycleMiddleware())
```

This pattern is useful for:
- Tracking active client connections
- Managing per-session resources (database connections, file handles)
- Logging connection/disconnection events
- Persisting session data on disconnect
- Cleanup operations when clients leave
